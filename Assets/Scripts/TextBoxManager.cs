   ...
   // public Shotgun shotgun;
    ...
    void Start()
    {
       ...
       // shotgun = FindObjectOfType<Shotgun>(); //setting up to stop the player from using shotgun
        ...
        if (textFile != null)
        { //if there's a text file that exists...
        ...
        if (isActive)
        { //if the textbox is active, enable it; otherwise, disable it
            EnableTextBox();
        }
        else
        {
    ...
    { //setting up a coroutene, which works in it's own timeline, in a way (runs at the same time as everything else)
        int letter = 0;
        theText.text = ""; //display nothing in the box
        isTyping = true;
        cancelTyping = false;
        while (isTyping && !cancelTyping && (letter < lineOfText.Length - 1))
        { //while the text is true/cancel text is false/the letter is one less than the length of the text...
